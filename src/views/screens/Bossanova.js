...
import BossanovaStudios from '../../consts/BossanovaStudios';
import BossanovaStudiosA from '../../consts/BossanovaStudiosA';
import BossanovaStudiosB from '../../consts/BossanovaStudiosB';
const bossanova = require('../screens/assets/Emueble3.jpg');
const map = require('../screens/assets/bossanovamap.jpg');
...
const Bossanova = ({route, navigation}) => {
  const Card = ({bossanovaetages}) => {
    return (
      <TouchableOpacity
      style={styles.cardImage}
        activeOpacity={0.8}
        onPress={() => navigation.navigate('BossanovaDetails', bossanovaetages)}>
  ...
  return (
    ...
    <View style={styles.container}>
      <ImageBackground source={bossanova} style={styles.backgroundImage}>
        <TouchableOpacity
        ...
        <View style={styles.titlesWrapper}>
          <Text style={styles.itemTitle}>Bossa Nova</Text>
          <View style={styles.locationWrapper}>
            <Entypo name="location-pin" size={24} color={colors.white} />
            <Text style={styles.locationText}>Gauthier – Casablanca</Text>
          </View>
        </View>
      </ImageBackground>
      ...
      <View style={styles.descriptionWrapper}>
        <View style={styles.descriptionTextWrapper}>
          ...
          <Text style={styles.descriptionText}>     Le projet Bossa Nova se situe au  {'\n'}    coeur du quartier Palmier, à l’arrière {'\n'}    du quartier des affaires Abdelmou {'\n'}   -men hassan 2,à proximité de toutes{'\n'}    les commodités. 
</Text>
</View>
     ...
     <Text style={styles.sectionTitle}>STUDIOS PAR ÉTAGE  </Text>
        <View>
        <Text style={styles.etageTitle}>R D C</Text>
          <FlatList
            contentContainerStyle={{paddingLeft: 20}}
            horizontal
            showsHorizontalScrollIndicator={false}
            data={BossanovaStudios}
            renderItem={({item}) => <Card bossanovaetages={item} />}
          />
          <Text style={styles.etageTitle}>1er ÉTAGE</Text>
          <FlatList
            contentContainerStyle={{paddingLeft: 20}}
            horizontal
            showsHorizontalScrollIndicator={false}
            data={BossanovaStudiosA}
            renderItem={({item}) => <Card bossanovaetages={item} />}
          />
          <Text style={styles.etageTitle}>2eme/3eme ÉTAGE</Text>
          <FlatList
            contentContainerStyle={{paddingLeft: 20}}
            horizontal
            showsHorizontalScrollIndicator={false}
            data={BossanovaStudiosB}
            renderItem={({item}) => <Card bossanovaetages={item} />}
          />
